...
public:
    ...
    explicit SearchServer(const StringContainer& stop_words)
        ...
        for (const string& word : stop_words) {
            if (!IsValidWord(word)) {
                throw invalid_argument("Недопустимые символы в конструкторе!"s);
            }
        }
    }
    ...
    void AddDocument(int document_id, const string& document, DocumentStatus status,
        ...
        if (document_id < 0) {
            throw invalid_argument("Переданный id меньше нуля!"s);
        }
        if (documents_.count(document_id) > 0) {
            throw invalid_argument("Документ с переданным id уже существует!"s);
        }
        vector<string> words;
    ...
    vector<Document> FindTopDocuments(const string& raw_query, DocumentPredicate document_predicate) const {
        ...
        if (!ParseQuery(raw_query, query)) {
            throw invalid_argument("Более одного символа '-' или отсутствие минус-слова после символа '-'!"s);
        }
        if (!IsValidWord(raw_query)) {
            throw invalid_argument("Недопустимые символы в запросе!"s);
        }
    ...
    int GetDocumentId(int index) const {
        // При выходе за границы, будет вызвано стандартное исключение out_of_range
...
int main()
{
    ...
    try {
        SearchServer search_server2("и скво\x12рец на"s);
    }
    catch (const exception& e) {
        cout << "Ошибка создания объекта SearchServer: "s << e.what() << endl;
    }
    ...
    {
        cout << "Ошибка обращения по индексу: "s << e.what() << endl;
    }
}
